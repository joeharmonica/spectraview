...
const STEPS: Step[] = [
  {
    ...
    body: 'A browser-based platform for loading, visualising, and processing spectral data. All data is saved automatically to your browser — nothing leaves your machine. Use ← → or keyboard arrows to navigate, Esc to exit.',
  },
  {
  ...
  },
  {
    ...
    body: 'Zoom (default): drag to draw a zoom box. Pan: drag to scroll the chart. Both support scroll-wheel zoom. Reset fits all loaded data back into view. Download saves the chart as a high-resolution PNG (1200 × 800 px, 2×). All controls also accept Plotly\'s built-in double-click to reset.',
  },
  {
    targetId: 'tutorial-peaks-btn',
    title: 'Peak Table',
    body: 'Detects local maxima across all selected spectra. Filter by prominence % (relative to the spectrum\'s maximum) or by absolute minimum intensity. Click the bookmark icon on any row to mark that peak on the chart with a labelled dotted line. Click "Clear markers" in the footer to remove all marks.',
  },
  {
    targetId: 'tutorial-labels-btn',
    title: 'Peak Annotations',
    body: 'Toggles name + wavelength callout labels at each spectrum\'s dominant peak directly on the chart. Useful for quick presentations. Labels are anchored above the peak and show the custom label (or filename) alongside the peak wavelength in nm.',
  },
  {
    targetId: 'tutorial-analysis-btn',
    title: 'Analysis Panel',
    body: 'Processing steps applied to selected spectra in order: (1) Crop — trim to a wavelength window. (2) Smooth — Savitzky-Golay filter with adjustable window and polynomial order. (3) Baseline — polynomial background subtraction. (4) Normalise — to maximum, unit area, or a reference wavelength. (5) Integrate — trapezoidal AUC between two wavelengths. Hover the ? icons for detailed tips.',
  },
  {
    targetId: 'tutorial-csv-btn',
    title: 'Export & Data Persistence',
    body: 'CSV exports the processed intensities for all selected spectra (one file per spectrum). Your loaded spectra and processing settings are automatically saved to IndexedDB in your browser — they\'ll be here next time you open the app. Use "Clear all" in the library to reset.',
  },
  {
    targetId: null,
    title: "You're all set!",
    body: 'Click the ? button in the header any time to replay this tour. Hover over the ? icons inside the Analysis panel for tips on each processing tool. On smaller screens, use the ☰ button in the header to open the library drawer.',
  },
];
      ...
      >
          ...
          {STEPS.map((_, i) => (
            <button
              ...
              className={`h-1 rounded-full transition-all duration-200
                ${i === stepIdx ? 'bg-blue-500 w-5' : i < stepIdx ? 'bg-blue-200 w-2' : 'bg-slate-200 w-2 hover:bg-slate-300'}`}
            />
          ))}
        </div>
        ...
        <div className="flex items-center justify-between px-5 pb-4">
          <button
            ...
            disabled={isFirst}
            className="text-xs text-slate-400 hover:text-slate-600 disabled:opacity-0 transition-colors"
          >
            ← Back
          </button>
          <button
            onClick={() => isLast ? onClose() : setStepIdx(i => i + 1)}
            className="px-5 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold rounded-lg transition-colors"
          >
            {isLast ? 'Finish' : 'Next →'}
          </button>
        </div>
